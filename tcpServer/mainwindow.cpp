...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    setWindowTitle("扫雷Server");//设置窗口标题
...
void MainWindow::on_pBtn_clicked()
{
    ...
    if(!server->listen(QHostAddress::Any, ui->pLine->text().toInt()))
    {
        qDebug() << server->errorString();
        server->close();
        QMessageBox::information(this, QString::fromLocal8Bit("Listen Failed!"),QString::fromLocal8Bit("Listen Failed!"));
        close();
        return ;
    }
    Listen=true;
    qDebug() << "ok : "<<ui->pLine->text().toInt();
    connect(server, &QTcpServer::newConnection, this, &MainWindow::getSocket);

    ui->pBtn->setText("wait");
    ui->pBtn->setEnabled("false");
    return ;
}
